...
const spawnEvents = key => utilActor.spawn(HANDLERS$1, key);
...
const spawnProviders = () => utilActor.spawn(HANDLERS, SERVICE_ACTOR_KEYS.AUTHN);
...
const makeHandlers = cfg => {
  ...
  const getStorageProvider = async () => {
    ...
    return await config.config.first(["fcl.storage", "fcl.storage.default"]);
  };
  return {
    [utilActor.INIT]: async ctx => {
...
const spawnCurrentUser = cfg => utilActor.spawn(makeHandlers(cfg), NAME);
